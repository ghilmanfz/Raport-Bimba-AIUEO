...
<div class="bg-white rounded-2xl p-6 border border-[#dee1e6] main-shadow flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
  <div class="flex items-start gap-4">
    <div class="w-12 h-12 bg-[#F97316]/10 rounded-xl flex items-center justify-center flex-shrink-0">
      <iconify-icon icon="lucide:file-text" width="22" class="text-[#F97316]"></iconify-icon>
    </div>
    <div>
  ...
  <div class="flex flex-col sm:flex-row items-center gap-3">
    <div class="relative w-full sm:w-64">
      ...
      <form method="GET" action="{{ route('guru.nilai') }}" id="form-filter">
        ...
        <select name="student_id" onchange="document.getElementById('form-filter').submit()" class="w-full pl-10 pr-10 py-2.5 bg-white border border-[#dee1e6] rounded-xl text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-[#F97316]/20">
...
<div class="border-b border-[#dee1e6] flex gap-8 mb-6">
  ...
  <button type="button" onclick="document.querySelector('#form-filter input[name=skill]').value='{{ $key }}'; document.getElementById('form-filter').submit();"
    class="flex items-center gap-2 px-2 py-3 border-b-2 font-semibold text-base {{ $selectedSkill === $key ? 'border-[#F97316] text-[#F97316]' : 'border-transparent text-[#565d6d] hover:text-[#171a1f]' }}">
...
<div class="bg-white rounded-2xl border border-[#dee1e6] main-shadow overflow-hidden mb-6">
  <div class="p-6 border-b border-[#dee1e6] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    ...
    <button type="submit" class="px-6 py-2.5 bg-[#F97316] text-white rounded-xl text-sm font-medium shadow-md hover:bg-orange-600 flex items-center gap-2">
    ...
    <table class="w-full text-left border-collapse">
      ...
      <tbody class="divide-y divide-[#dee1e6]">
        ...
        <tr class="hover:bg-gray-50/50">
          ...
          <td class="px-6 py-4">
            <input type="date" name="progress[{{ $i }}][start_date]" value="{{ $p['start_date'] }}" class="w-full px-3 py-2 border border-[#dee1e6] rounded-lg text-sm focus:ring-1 focus:ring-[#F97316] outline-none">
          </td>
          <td class="px-6 py-4">
            <input type="date" name="progress[{{ $i }}][understand_date]" value="{{ $p['understand_date'] }}" class="w-full px-3 py-2 border border-[#dee1e6] rounded-lg text-sm focus:ring-1 focus:ring-[#F97316] outline-none">
          </td>
          <td class="px-6 py-4">
