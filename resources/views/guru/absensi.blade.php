...
@section('title', 'Absensi Siswa - E-Rapor BiMBA')
@section('page-title', 'Absensi Siswa')

@section('content')
<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
  <div><h1 class="text-2xl lg:text-3xl font-bold tracking-tight">Absensi Siswa</h1><p class="text-sm text-[#565d6d] mt-1">Catat kehadiran harian siswa bimbingan Anda.</p></div>
...
<form method="GET" action="{{ route('guru.absensi.index') }}" class="bg-white rounded-2xl border border-[#dee1e6] main-shadow p-5 mb-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
  <div class="md:col-span-4"><label for="attendance-date" class="block text-xs font-semibold text-[#565d6d] mb-1.5">Tanggal</label><input id="attendance-date" type="date" name="attendance_date" value="{{ $date }}" max="{{ now()->toDateString() }}" class="w-full px-3 py-2.5 border border-[#dee1e6] rounded-xl text-sm"></div>
  <div class="md:col-span-5"><label for="attendance-search" class="block text-xs font-semibold text-[#565d6d] mb-1.5">Cari Siswa</label><input id="attendance-search" type="text" name="search" value="{{ $search }}" placeholder="Nama atau NIS" class="w-full px-3 py-2.5 border border-[#dee1e6] rounded-xl text-sm"></div>
...
<form method="POST" action="{{ route('guru.absensi.store') }}">
  ...
  <div class="bg-white rounded-2xl border border-[#dee1e6] main-shadow overflow-hidden">
    ...
    <div class="overflow-x-auto"><table class="w-full text-left min-w-[760px]"><thead class="bg-[#fafafb] border-b border-[#dee1e6] text-xs uppercase text-[#565d6d]"><tr><th class="px-5 py-4">Siswa</th><th class="px-5 py-4">Kelas</th><th class="px-5 py-4 w-52">Kehadiran</th><th class="px-5 py-4">Catatan</th></tr></thead><tbody class="divide-y divide-[#edf0f3]">
      ...
      @empty <tr><td colspan="4" class="px-6 py-12 text-center text-sm text-[#565d6d]">Tidak ada siswa bimbingan yang sesuai.</td></tr> @endforelse
    </tbody></table></div>
