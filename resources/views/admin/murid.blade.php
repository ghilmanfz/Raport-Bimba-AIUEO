...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
  ...
  <div class="flex flex-wrap items-center gap-3">
    ...
    <button onclick="openMuridModal('modal-tambah-murid')" class="flex items-center gap-2 px-4 py-2 bg-[#3d8af5] text-white rounded-xl font-medium text-sm hover:bg-blue-600">
...
<div class="bg-white rounded-2xl border border-[#dee1e6] overflow-hidden main-shadow">
  <div class="p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[#dee1e6]">
    <h2 class="text-xl font-semibold tracking-tight">Daftar Murid</h2>
    <form method="GET" action="{{ route('admin.murid') }}" class="flex flex-wrap items-center gap-2 w-full lg:w-auto">
      <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama/NIS" class="px-4 py-2 bg-white border border-[#dee1e6] rounded-xl text-sm">
      <select name="status" class="px-4 py-2 bg-white border border-[#dee1e6] rounded-xl text-sm">
        <option value="">Semua Status</option>
        <option value="aktif" {{ request('status') === 'aktif' ? 'selected' : '' }}>Aktif</option>
        <option value="lulus" {{ request('status') === 'lulus' ? 'selected' : '' }}>Lulus</option>
        <option value="pindah" {{ request('status') === 'pindah' ? 'selected' : '' }}>Pindah</option>
      </select>
      <select name="teacher_id" class="px-4 py-2 bg-white border border-[#dee1e6] rounded-xl text-sm">
        ...
        <tr class="hover:bg-gray-50/50">
          ...
          <td class="px-6 py-4 text-right">
            <div class="flex items-center justify-end gap-1">
              ...
              <button onclick="openEditMurid({{ $student->id }}, '{{ addslashes($student->name) }}', '{{ $student->nis }}', '{{ $student->classroom_id }}', '{{ $student->teacher_id }}', '{{ $student->join_date->format('Y-m-d') }}', '{{ $student->status }}', '{{ $student->parent_id ?? '' }}')" class="p-2 text-[#3d8af5] hover:bg-blue-50 rounded-lg" title="Edit">
...
<div id="modal-tambah-murid" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center">
    ...
    <form method="POST" action="{{ route('admin.murid.store') }}" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="name" required class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="Nama Lengkap">
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <select name="classroom_id" required class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
        ...
        <div>
          ...
          <input type="date" name="join_date" required value="{{ date('Y-m-d') }}" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <select id="teacher-select" name="teacher_id" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" onchange="checkTeacherCapacity(this)">
        ...
        <div>
          ...
          <select name="status" required class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
      ...
      <div class="flex gap-3 mb-4">
        <label class="flex items-center gap-2 px-3 py-2 border border-[#dee1e6] rounded-xl cursor-pointer hover:bg-blue-50 has-checked:bg-blue-50 has-checked:border-blue-400">
      ...
      <div id="wali-pilih-section" class="space-y-4">
        <div>
          ...
          <select name="parent_id" id="parent_id_select" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
      ...
      <div id="wali-buat-section" class="space-y-4 hidden">
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="father_name" id="father_name_input" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="Nama Ayah">
          </div>
          <div>
            <label class="block text-sm font-medium text-[#565d6d] mb-1">Nama Ibu</label>
            <input type="text" name="mother_name" id="mother_name_input" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="Nama Ibu">
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="father_phone" id="father_phone_input" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="08xxxxxxxxxx">
          </div>
          <div>
            <label class="block text-sm font-medium text-[#565d6d] mb-1">Kontak Ibu (Opsional)</label>
            <input type="text" name="mother_phone" id="mother_phone_input" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="08xxxxxxxxxx">
          </div>
        </div>

        <div>
          <label class="block text-sm font-medium text-[#565d6d] mb-1">Alamat</label>
          <textarea name="address" id="address_input" rows="2" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="Alamat rumah"></textarea>
        </div>

        <div>
          <label class="block text-sm font-medium text-[#565d6d] mb-1">Email Wali (Opsional)</label>
          <input type="email" name="parent_email" id="parent_email_input" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" placeholder="wali@email.com">
          <p class="mt-1 text-xs text-[#565d6d]">Password default: password123</p>
        </div>
      </div>

      <div class="flex gap-3 pt-2">
        <button type="button" onclick="closeMuridModal('modal-tambah-murid')" class="flex-1 py-2.5 border border-[#dee1e6] rounded-xl text-sm font-medium text-[#565d6d]">Batal</button>
        <button type="submit" class="flex-1 py-2.5 bg-[#3d8af5] text-white rounded-xl text-sm font-medium">Simpan</button>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="bg-white rounded-2xl p-8 w-full max-w-lg mx-4 shadow-xl">
    ...
    <form id="form-edit-murid" method="POST" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-[#565d6d] mb-1">NIS</label>
          <input type="text" name="nis" id="edit-murid-nis" required class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
        </div>
        <div>
          <label class="block text-sm font-medium text-[#565d6d] mb-1">Nama Murid</label>
          <input type="text" name="name" id="edit-murid-name" required class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-[#565d6d] mb-1">Tahapan</label>
          <select name="classroom_id" id="edit-murid-classroom" required class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
            @foreach($classrooms as $classroom)
              <option value="{{ $classroom->id }}">{{ $classroom->name }}</option>
            @endforeach
          </select>
        ...
        <div>
          <label class="block text-sm font-medium text-[#565d6d] mb-1">Guru Pembimbing</label>
          <select name="teacher_id" id="edit-murid-teacher" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm" onchange="checkEditTeacherCapacity(this)">
            <option value="">Tidak Ada</option>
            @foreach($teachers as $teacher)
              <option value="{{ $teacher->id }}" data-student-count="{{ $teacher->students()->count() }}">{{ $teacher->user->name }} ({{ $teacher->students()->count() }}/25)</option>
            @endforeach
      ...
      <div>
        ...
        <select name="parent_id" id="edit-murid-parent" class="w-full px-4 py-2 border border-[#dee1e6] rounded-xl text-sm">
          <option value="">— Tidak Ada —</option>
          @foreach($guardians as $guardian)
      ...
      <div class="flex gap-3 pt-2">
        <button type="button" onclick="closeMuridModal('modal-edit-murid')" class="flex-1 py-2.5 border border-[#dee1e6] rounded-xl text-sm font-medium text-[#565d6d]">Batal</button>
        <button type="submit" class="flex-1 py-2.5 bg-[#3d8af5] text-white rounded-xl text-sm font-medium">Perbarui</button>
      </div>
    </form>
  </div>
</div>
...
function openMuridModal(id) {
  ...
  // Reset wali option to pilih
  document.querySelector('input[name="wali_option"][value="pilih"]').checked = true;
