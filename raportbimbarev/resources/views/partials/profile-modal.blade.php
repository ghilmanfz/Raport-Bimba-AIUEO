  ...
  <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" @click.stop>
    ...
    <div class="sticky top-0 bg-white border-b border-[#dee1e6] p-6 pb-4 rounded-t-2xl z-10">
      ...
      <div class="flex gap-4 mt-4 border-b border-[#dee1e6] -mb-4">
        <button @click="activeTab = 'info'" :class="activeTab === 'info' ? 'border-[#F97316] text-[#F97316]' : 'border-transparent text-[#565d6d]'" class="pb-3 border-b-2 text-sm font-semibold">
          Informasi Akun
        </button>
        <button @click="activeTab = 'password'" :class="activeTab === 'password' ? 'border-[#F97316] text-[#F97316]' : 'border-transparent text-[#565d6d]'" class="pb-3 border-b-2 text-sm font-semibold">
          Ganti Password
        </button>
      </div>
    </div>
    ...
    <div x-show="activeTab === 'info'" class="p-6">
      <form method="POST" action="{{ route('profile.update') }}" id="profile-info-form">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input type="text" name="name" value="{{ auth()->user()->name }}" required
              class="w-full px-4 py-2.5 border border-[#dee1e6] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#F97316]/20">
          </div>
          <div>
        ...
        <div class="mt-6 flex justify-end gap-3">
          ...
          <button type="submit" class="px-5 py-2.5 bg-[#F97316] text-white rounded-xl text-sm font-medium shadow-md hover:bg-orange-600">Simpan Perubahan</button>
        </div>
      </form>
    </div>
    ...
    <div x-show="activeTab === 'password'" class="p-6">
      <form method="POST" action="{{ route('profile.password') }}" id="profile-password-form">
        ...
        <div class="mt-6 flex justify-end gap-3">
          <button type="button" @click="showProfileModal = false" class="px-5 py-2.5 border border-[#dee1e6] rounded-xl text-sm font-medium text-[#565d6d] hover:bg-gray-50">Batal</button>
          <button type="submit" class="px-5 py-2.5 bg-[#F97316] text-white rounded-xl text-sm font-medium shadow-md hover:bg-orange-600">Ganti Password</button>
        </div>
      </form>
    </div>
  </div>
